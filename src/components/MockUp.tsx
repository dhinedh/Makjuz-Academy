...
const MockUp: React.FC = () => {
  ...
  return (
    <div
      className={`flex min-h-screen relative overflow-hidden transition-colors duration-500 ${
        isDarkMode
    ...
    >
      ...
      <motion.aside
        ...
        className={`relative z-10 backdrop-blur-2xl border-r transition-all duration-300 ${
          isDarkMode
      ...
      >
        {/* Sidebar Header */}
        <div className="flex items-center justify-between h-20 px-6 border-b border-opacity-20">
          <AnimatePresence>
            {isOpen && (
              <motion.div
      ...
      <main className="flex-1 flex flex-col justify-center items-center text-center px-8 relative z-10">
        {/* Hero Badge */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          ...
          <motion.button
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            className={`px-8 py-4 rounded-2xl font-semibold text-lg backdrop-blur-xl transition-all duration-300 border ${
              isDarkMode
